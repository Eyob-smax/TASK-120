...
test.describe('Role navigation and core task closure', () => {
  test('first-run setup creates admin, navigates to dashboard, and shows nav items', async ({ page }) => {
    // 1. Load the app — first run shows account creation
    await page.goto('/');
    await expect(page.locator('h1')).toHaveText('ForgeOps');
    await expect(page.locator('.subtitle')).toContainText('Create your administrator account');
  ...
  test('admin can navigate to Inventory page and see content', async ({ page }) => {
  ...
  test('admin can navigate to Files page and see version drawer', async ({ page }) => {
    await page.goto('/');
    await page.fill('input[placeholder="Your name"]', 'Test Admin');
...
test.describe('Accessibility: semantic structure and ARIA', () => {
  ...
  async function setupAdmin(page: import('@playwright/test').Page, username: string) {
    await page.goto('/');
  ...
  test('login form uses proper form element and required attributes', async ({ page }) => {
    await page.goto('/');
  ...
  test('nav rail uses semantic <nav> element with aria-labeled controls', async ({ page }) => {
  ...
  test('toast container has aria-live for dynamic announcements', async ({ page }) => {
