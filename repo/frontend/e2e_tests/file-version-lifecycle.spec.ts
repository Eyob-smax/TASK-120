...
async function uploadFile(page: Page, fileName: string, content: string) {
  ...
  await expect(page.locator('[role="status"]')).toContainText('uploaded');
...
test.describe('File version lifecycle — real workflows', () => {
  test('upload file via modal, verify file appears in data table', async ({ page }) => {
    await setupAdmin(page, 'file-real-upload-1');
    await navigateToFiles(page);

    await uploadFile(page, 'test-doc.txt', 'Hello, this is test content for E2E');
  ...
  test('delete file moves to recycle bin, restore returns it', async ({ page }) => {
    ...
    await expect(page.locator('[role="status"]')).toContainText('recycle bin');

    // File should no longer be in main table
    await expect(table.locator('td:has-text("deletable.txt")')).not.toBeVisible();
    ...
    await expect(page.locator('strong:has-text("deletable.txt")')).toBeVisible();

    // Restore the file
    await page.click('button:has-text("Restore")');
    await expect(page.locator('[role="status"]')).toContainText('restored');
  ...
  test('recycle bin shows purge expired button and handles empty state', async ({ page }) => {
    ...
    await page.click('button:has-text("Recycle Bin")');
    await expect(page.locator('h4:has-text("Recycle Bin")')).toBeVisible();
    await expect(page.locator('button:has-text("Purge Expired")')).toBeVisible();

    // Click purge — should succeed even with 0 expired entries
    await page.click('button:has-text("Purge Expired")');
    await expect(page.locator('[role="status"]')).toContainText('Purged');
  });

  test('version drawer shows version history after upload', async ({ page }) => {
