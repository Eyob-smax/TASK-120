...
async function receiveStock(
  ...
  await expect(page.locator('[role="status"]')).toContainText(`Received ${qty} units`);
}

test.describe('Inventory workflows', () => {
  test('receive stock creates inventory record and shows in table', async ({ page }) => {
  ...
  test('ship stock deducts quantity from inventory', async ({ page }) => {
    ...
    await expect(page.locator('[role="status"]')).toContainText('Shipped');
    const table = page.locator('table.data-table');
  ...
  test('transfer stock moves quantity between bins', async ({ page }) => {
    ...
    await expect(page.locator('[role="status"]')).toContainText('Transferred');
  ...
  test('cycle count adjusts stock and records ledger entry', async ({ page }) => {
    ...
    await expect(page.locator('[role="status"]')).toContainText('Cycle count');
    ...
    await expect(page.locator('td:has-text("cycle_count_adjust")')).toBeVisible();
  });

  test('receive low stock triggers safety stock alert banner', async ({ page }) => {
