...
export async function optimisticReceive(
  ...
  const optimistic = match
    ? current.map((r) =>
        r === match ? { ...r, quantity: r.quantity + quantity } : r,
      )
    : [
        ...current,
        {
  ...
  return createOptimisticUpdate(inventoryStore, optimistic, async () => {
    const { alerts } = await _receiveStock(
      binId,
      skuId,
      warehouseId,
      quantity,
...
export async function optimisticShip(
  ...
  const optimistic = current.map((r) =>
    r.binId === binId && r.skuId === skuId
      ? { ...r, quantity: r.quantity - quantity }
      : r,
  );

  return createOptimisticUpdate(inventoryStore, optimistic, async () => {
    const { alerts } = await _shipStock(
      binId,
      skuId,
      warehouseId,
      quantity,
...
export async function optimisticTransfer(
  ...
  const current = get(inventoryStore);
  const optimistic = current.map((r) => {
    if (r.binId === fromBinId && r.skuId === skuId)
      return { ...r, quantity: r.quantity - quantity };
    if (r.binId === toBinId && r.skuId === skuId)
      return { ...r, quantity: r.quantity + quantity };
    return r;
  });

  return createOptimisticUpdate(inventoryStore, optimistic, async () => {
    const { alerts } = await _transferStock(
      fromBinId,
      toBinId,
      skuId,
      quantity,
...
export async function optimisticCycleCount(
  ...
  return createOptimisticUpdate(inventoryStore, optimistic, async () => {
    const { alerts } = await _cycleCount(
      binId,
      skuId,
      warehouseId,
      actualQty,
