<script lang="ts">
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      error = e.message || 'Failed to receive stock';
    } finally {
      submitting = false;
    }
  }

  function resetForm() { binId = ''; skuId = ''; warehouseId = ''; quantity = 0; notes = ''; error = ''; }
</script>

<Modal {open} title="Receive Stock" confirmLabel={submitting ? 'Saving...' : 'Receive'} confirmDisabled={submitting} on:close={() => { open = false; }} on:confirm={handleConfirm}>
