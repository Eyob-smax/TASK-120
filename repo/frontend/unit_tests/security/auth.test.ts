...
describe('Auth Service', () => {
  ...
  it('createUser works when logged in as Administrator', async () => {
    await createInitialAdmin('admin', 'AdminPass1', testProfile);
    await login('admin', 'AdminPass1');
  ...
  it('createUser throws when not Administrator', async () => {
    await createInitialAdmin('admin', 'AdminPass1', testProfile);
    await login('admin', 'AdminPass1');
    await createUser('worker1', 'Worker1Pass', UserRole.PickerPacker, { displayName: 'W1' });
    logout();

    await login('worker1', 'Worker1Pass');
    await expect(
      createUser('worker2', 'Worker2Pass', UserRole.PickerPacker, { displayName: 'W2' }),
