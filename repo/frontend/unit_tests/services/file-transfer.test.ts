...
import {
  ...
  rollbackToVersion,
  getVersionHistory,
} from "../../src/modules/files/version.service";
import {
  deleteFile,
  restoreFile,
  purgeExpired,
} from "../../src/modules/files/recycle-bin.service";
import {
  FileRepository,
  VersionRepository,
...
describe("File Service", () => {
  ...
  describe("ingestFile", () => {
    ...
    it("deduplicates identical files", async () => {
      ...
      const { deduplicated } = await ingestFile(
        "second.txt",
        512,
        "text/plain",
        data,
...
describe("ChunkScheduler", () => {
  ...
  it("processChunk stores versionId on chunk when provided", async () => {
    ...
    try {
      const scheduler = new ChunkScheduler();
      const chunk = await scheduler.processChunk(
        crypto.randomUUID(),
        0,
        new ArrayBuffer(64),
        "ver-xyz",
  ...
  it("scheduleChunks with versionId writes new chunks despite prior version chunks existing", async () => {
    ...
    try {
      ...
      const { file, session: s1 } = await ingestFile(
        "iso-test.bin",
        data1.byteLength,
        "application/octet-stream",
        data1,
      ...
      const { version: v1 } = await createVersion(
        file.id,
        file.sha256,
        file.size,
        "u1",
      );
      await new ChunkScheduler().scheduleChunks(
        s1!.id,
        data1,
        undefined,
        v1.id,
  ...
  it("resumed scheduleChunks completes version-tagged chunks without re-writing existing ones", async () => {
    ...
    try {
      ...
      const numChunks = 6;
      const fileData = makeFileData(FILE_CHUNK_SIZE * numChunks);
      const { file, session } = await ingestFile(
        "resume-v.bin",
        fileData.byteLength,
        "application/octet-stream",
        fileData,
      ...
      // Keep this stress test under Vitest's per-test timeout in slower CI/Docker runs.
      sched1.setBandwidthCap(100 * 1024 * 1024);
      let count = 0;
      const origProcess = sched1.processChunk.bind(sched1);
      vi.spyOn(sched1, "processChunk").mockImplementation(
        async (fid, idx, data, vid) => {
      ...
      await resumeTransfer(session!.id);
      const sched2 = new ChunkScheduler();
      sched2.setBandwidthCap(100 * 1024 * 1024);
      await sched2.scheduleChunks(session!.id, fileData, undefined, v1.id);
...
describe("Recycle Bin Service", () => {
  ...
  it("purges entries older than 30 days", async () => {
    ...
    const futureDate = new Date(
      Date.now() + RECYCLE_BIN_RETENTION_MS + 86_400_000,
    );
